...
vips_foreign_save_jp2k_dispose(GObject *gobject)
{
	...
	G_OBJECT_CLASS(vips_foreign_save_jp2k_parent_class)->dispose(gobject);
}
...
#define SHRINK(OUTPUT_TYPE, ACC_TYPE, PIXEL_TYPE) \
	{ \
		...
		const int dy = VIPS_MIN(comp->dy, tile->height - y * comp->dy); \
\
		PIXEL_TYPE *tp; \
		ACC_TYPE *ap; \
\
...
\
		...
		for (x = 0; x < output_width; x++) { \
			const int dx = VIPS_MIN(comp->dx, tile->width - x * comp->dx); \
